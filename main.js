...
function render() {
  ...
  todos.forEach((item) => {
    ...
    <span class = "square" >${item.name}</span>x
    <div>
    <div>
...
document.addEventListener("click", (e) => {
  if (e.target.classList.contains("edit-btn")) {
    ...
    editInput.value = todoToEdit.surname;
    editInput2.value = todoToEdit.number;
    editInput3.value = todoToEdit.name;
    editInput4.value = todoToEdit.email;
    editInput.focus();
...
editSubmit.addEventListener("click", (e) => {
  ...
  //? перебираем todos и меняем title у нужного элемента на содержимое инпута
  todos = todos.map((item) => {
    if (item.id == editId) {
      item.surname= editInput.value;
      item.number = editInput2.value;
      item.name = editInput3.value;
      item.email= editInput4.value;
    }
    return item;
  });

  //? сохраняем в localStorage актуальные данные
  localStorage.setItem("todos", JSON.stringify(todos));
